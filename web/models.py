...
.model-card{padding:22px;background:var(--panel);border:1px solid var(--line);border-radius:12px}.model-card h2{font-size:21px;line-height:1.3;margin:12px 0}.model-card h2 a{text-decoration:none;color:var(--ink)}
...
SCRIPT = '''<script>
const search=document.querySelector('#model-search'), kind=document.querySelector('#model-kind');
function filter(){let count=0;document.querySelectorAll('[data-model]').forEach(row=>{row.hidden=!(row.textContent.toLowerCase().includes(search.value.toLowerCase())&&(!kind.value||row.dataset.kind===kind.value));if(!row.hidden)count++;});document.querySelector('#model-count').textContent=count+' models shown';document.querySelector('#model-empty').hidden=count!==0;}
search.addEventListener('input',filter);kind.addEventListener('change',filter);
</script>'''


def listing(catalog, shell, *, archive=False):
  models = [m for m in catalog.models(include_archive=True) if m['archived'] == archive]
  cards = []
  for m in models:
    formats = sorted({f for v in m['variants'] for f in v['formats']})
    cards.append(f'''<article class="model-card" data-model data-kind="{e(m['kind'], quote=True)}">
  ...
  title = 'Model archive' if archive else 'Explore driving models'
  intro = 'Historical training runs and source configurations, preserved for research and integration.' if archive else 'Find named models and their source configurations for openpilot forks.'
  ...
  return shell(title + ' — openmodels', STYLE + f'''<main><section class="hero"><h1>{title}</h1><p>{intro}</p>
    ...
    <label for="model-kind">Model type<select id="model-kind"><option value="">All types</option>{options}</select></label></div>
    <p id="model-count" role="status">{len(models)} models shown · newest source activity first</p>
...
def detail(catalog, model, shell, *, base_url=""):
  ...
  return shell(model['name'] + ' — openmodels', STYLE + f'''<main><a href="{'archive.html' if model['archived'] else 'index.html'}">← All models</a>
    <section class="hero"><p>{e(model['publisher'])} · {e(model['kind'])} · {e(model['family'])}</p><h1>{e(model['name'])}</h1><p>{e(model['description'])}</p></section>
    <ul>{links}</ul><h2>Available configurations</h2><p>Each configuration preserves a specific source context. Your fork decides which configurations it supports and how to activate them. <a href="integrate.html">Integration guide</a></p>
    {''.join(variants)}</main>''')
