...
<header class="top">
<a class="brand" href="index.html" aria-label="OpenModels directory"><svg viewBox="0 0 32 32" width="28" height="28" aria-hidden="true"><path d="M8 8h16v16H8zM8 16h16M16 8v16" fill="none" stroke="currentColor" stroke-width="2"/><path d="M4 8h8v8H4zM20 20h8v8h-8z" fill="currentColor"/></svg>openmodels</a>
<nav class="top" aria-label="Main navigation"><a href="index.html">Directory</a><a href="archive.html">Archive</a><a href="integrate.html">Developers</a></nav>
<button id="theme-toggle" type="button" aria-label="Switch to light theme">Light theme</button>
</header>
<div id="content" tabindex="-1">{body}</div>
<footer><span>Open models. Explicit configurations.</span><div><a href="catalog.json">Catalog JSON</a><a href="https://github.com/jjolano/openmodels">GitHub</a></div>
<details><summary>Provenance and licenses</summary><p>{DISCLAIMER}</p><p>Comma models © comma.ai under the <a href="https://github.com/commaai/openpilot/blob/master/LICENSE">MIT license</a>. Not affiliated with comma.ai. <a href="https://sunnylink.wiki/models">Community model descriptions</a>.</p></details></footer>
</div><script>
...
const path=location.pathname.split('/').pop();const active=(!path||path==='index.html'||/^index-\\d+\\.html$/.test(path))?'index.html':path==='integrate.html'?'integrate.html':'archive.html';document.querySelectorAll('nav.top a').forEach(a=>{{if(a.getAttribute('href')===active)a.setAttribute('aria-current','page')}});
</script></body></html>"""
...
def render(index_path: Path, out_dir: Path) -> int:
  ...
  catalog = publish(index, out_dir, blob_base="blobs" if BLOB_BACKEND == "local" else None)
